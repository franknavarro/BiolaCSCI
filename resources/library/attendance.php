...
class attendance{
        ...
        if(!empty($result)){
            switch ($sessionStatus) {
                ...
                case 1:
                    ...
                    $nameResult = db::query("SELECT sessionName from session WHERE sessionID=:sessionID", array(':sessionID'=>$sessionID));
                    $className = print_r($nameResult[0]['sessionName'], true);
                    return $className; //Returns class name for user
                    break;
                    //Note: Tardy; 2
                case 2:
    ...
    public static function createClass($classID, $hostEmail){
    ...
    public static function startClass($sessionID){
    ...
    public static function endClass($sessionID){
        $result = db::query("SELECT sessionName from session WHERE sessionID=:sessionID", array(':sessionID' => $sessionID));
        if(!empty($result)){
            $className = print_r($result[0]['sessionName']);
            $classID = db::query("SELECT classID from session WHERE className=:className", array(':className' => $className));
