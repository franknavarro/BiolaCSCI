...
?>
    ...
    <div id="class-info-page" class="class-section" style="display:block;">
        ...
        <h5 class="cinfo-header sub-header">
            ...
            <?php
                $taNameQuery = db::query("SELECT firstName, lastName from user
                                          LEFT JOIN user_class ON user.email = user_class.user_email
                                          AND user_class.role = 2
                                          AND user_class.class_classID = :currentClass;", array(':currentClass'=>$currentClass));

                echo $taNameQuery[0]["firstName"] . " " . $taNameQuery[0]["lastName"];
            ?>
        </h5>
        <p><strong>Email: </strong>
            ...
            <a href="mailto:
            <?php
                $taEmail = db::query("SELECT user_email FROM user_class WHERE role = 2 AND class_classID = :classID;", array(':classID'=>$currentClass));
                print_r($taEmail[0][0]);
            ?>">
            <?php
                print_r($taEmail[0][0]);
